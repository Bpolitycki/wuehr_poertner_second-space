...
const zoomIn = () => {
    /*@ts-ignore*/
    select(g.value as SVGGElement).transition().call(imgZoom.scaleBy as any, 2);
}

const zoomOut = () => {
    /*@ts-ignore*/
    select(g.value as SVGGElement).transition().call(imgZoom.scaleBy as any, 0.5);
}
...
<template>
    <div class="has-background-light  pt-0">
        <div class="buttons are-normal has-addons has-background-grey p-0 is-justify-content-space-between">
            ...
            item.metadata.files.length)"><span class=" icon">
                    <IconNext />
                </span>
            </button>
        </div>
        <div class="p-2">
            <svg ref="svg" :viewBox="viewBox"
                :style="{ width: width + '%' || w + 'px', height: '100%', maxHeight: '100%' }">
                <g ref="g"
                    :style="{ scale: transformStyles.scale, transform: `translate(${transformStyles.translateX}px, ${transformStyles.translateY}px)` }">
